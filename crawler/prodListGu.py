...
def getGuProd(driver,userName,password):
    ...
    for page in range(manProdPage):
        manQstart = page * 48
        res = requests.get('https://www.gu-global.com/tw/store/search?qbrand=20&qclv1=001&qstart=' + str(manQstart),
                           headers=headers)
        BS = BeautifulSoup(res.text, "lxml")
    ...
    for prod_url in manProdUrlSet.copy():
        print(str(len(manProdUrlSet)) + ' items left for man in Gu')
        try:
            getProdInfo(prod_url, driver, 'gu_clothes', userName, password)
        except Exception as ec:
            exceptionFormat(ec, prod_url)
        else:
            manProdUrlSet.remove(prod_url)
    for prod_url in womanProdUrlSet.copy():
        print(str(len(womanProdUrlSet)) + ' items left for woman in Gu')
        try:
            getProdInfo(prod_url, driver, 'gu_clothes', userName, password)
        except Exception as ec:
            exceptionFormat(ec, prod_url)
