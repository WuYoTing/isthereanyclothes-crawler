...
def get_gu_prod(driver, db_host, db_name, db_user_name, db_password, mail_user, mail_password, recipient):
    user_agent = random.choice(user_agent_list)
    headers = {'user-agent': user_agent, "accept-language": "zh-TW,zh;q=0.9,en-US;q=0.8,en;q=0.7"}
    # 取男生所有產品
    man_prod_url_set = set()
    man_index_res = requests.get('https://www.gu-global.com/tw/store/search?qbrand=20&qclv1=001', headers=headers)
    ...
    for page in range(man_prod_page):
        ...
        man_page_res = requests.get(
            'https://www.gu-global.com/tw/store/search?qbrand=20&qclv1=001&qstart=' + str(man_page_index),
            headers=headers)
    ...
    woman_index_res = requests.get('https://www.gu-global.com/tw/store/search?qbrand=20&qclv1=002', headers=headers)
    ...
    for page in range(woman_prod_page):
        ...
        woman_page_res = requests.get(
            'https://www.gu-global.com/tw/store/search?qbrand=20&qclv1=002&qstart=' + str(woman_page_index),
            headers=headers)
    ...
    for prod_url in man_prod_url_set.copy():
        ...
        try:
            get_prod_info(prod_url, driver, 'gu_clothes', db_host, db_name, db_user_name, db_password)
